 ...
 * 检查文件是否应该被排除
 */
function shouldExcludeFile(filePath: string, buildDest: string, userExcludeFiles: string[]): { excluded: boolean; reason?: string } {
 ...
 * 构建开始前的钩子
 */
export const onBeforeBuild = async function (options: any, result?: any) {
...
export const onAfterBuild = async function (options: any, result?: any) {
    ...
    const selectedPreset = PRESETS[preset as keyof typeof PRESETS] || PRESETS.medium;
    ...
    const reports: ObfuscationReport[] = [];
    ...
    for (const jsFile of jsFiles) {
        ...
        try {
            ...
            // 执行混淆
            const obfuscationResult = JavaScriptObfuscator.obfuscate(originalCode, selectedPreset);
 ...
 */
async function findJsFiles(dir: string, excludeDirs: string[]): Promise<string[]> {
 ...
 * 压缩设置文件前的钩子
 */
export const onBeforeCompressSettings = async function (options: any, result?: any) {
 ...
 * 压缩设置文件后的钩子
 */
export const onAfterCompressSettings = async function (options: any, result?: any) {
